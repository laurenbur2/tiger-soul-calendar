...
.bg{position:absolute;inset:0;background-image:url(src5596.jpg);background-repeat:no-repeat}
/* light green fade: photo stays open, colour only deepens under the type */
.scrim{position:absolute;inset:0;background:linear-gradient(180deg,
 rgba(15,28,20,.80) 0%, rgba(15,28,20,.50) 13%, rgba(15,28,20,0) 30%,
...
.q{font-family:var(--disp);font-weight:400;font-size:96px;line-height:1.10;color:var(--cream);
...
"""
QUOTE = "&ldquo;Have you ever<br>experienced a 5-MeO<br>reactivation?&rdquo;"
SUB   = "Reactivations"
...
# portrait source 2756x4128 -> 119% is exact cover. larger = zoomed in.
# Y=0% keeps his head as low in the frame as the source allows (clear of the logo).
for name,size,pos in [("F","150%","55% 0%")]:
    hp=f"react-{name}.html"; open(hp,"w").write(doc(size,pos))
    png=f"react-{name}.png"
    ...
    subprocess.run(["sips","-s","format","jpeg","-s","formatOptions","95",png,"--out",f"react-{name}.jpg"],
