...
.bg{position:absolute;inset:0;background-image:url(bg-kambo.jpg);background-repeat:no-repeat}
/* light green fade: photo stays open, colour only deepens under the type */
.scrim{position:absolute;inset:0;background:linear-gradient(180deg,
 rgba(15,28,20,.55) 0%, rgba(15,28,20,.18) 12%, rgba(15,28,20,0) 30%,
...
.blk{position:absolute;left:0;right:0;bottom:320px;padding:0 78px;text-align:center}
...
"""
QUOTE = "&ldquo;Kambo healed my<br>crazy infection&rdquo;"
SUB   = "Kambo"
...
# source is 960x1280 (portrait). 133% == exact cover. larger = zoomed in.
for name,size,pos in [("infection","150%","42% 34%")]:
    hp=f"k-{name}.html"; open(hp,"w").write(doc(size,pos))
    png=f"cover-kambo-{name}.png"
    ...
    subprocess.run(["sips","-s","format","jpeg","-s","formatOptions","95",png,"--out",f"cover-kambo-{name}.jpg"],
      check=True,capture_output=True)
    os.remove(png); os.remove(hp); print("built",name)
