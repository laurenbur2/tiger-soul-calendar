...
.bg{position:absolute;inset:0;background-image:url(bg-depression.jpg);background-repeat:no-repeat}
...
.blk{position:absolute;left:0;right:0;bottom:300px;padding:0 78px;text-align:center}
.q{font-family:var(--disp);font-weight:400;font-size:92px;line-height:1.10;color:var(--cream);
...
"""
QUOTE = "&ldquo;Plant medicine helped<br>me heal my depression&rdquo;"
SUB   = "Ceremony"
...
# source is 1536x2048 (portrait 3:4). 125% == exact cover. larger = zoomed in.
for name,size,pos in [("depression","165%","40% 93%")]:
    hp=f"d-{name}.html"; open(hp,"w").write(doc(size,pos))
